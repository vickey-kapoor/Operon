...
`RuleBasedRecoveryManager` in `src/agent/recovery.py` maps failed or uncertain steps to recovery decisions. The ladder is keyed by a retry cluster built from subgoal, target, and failure signal.
