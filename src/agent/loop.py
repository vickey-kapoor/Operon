...
class AgentLoop:
    ...
    def _tag_input_zone(self, decision, perception) -> "PolicyDecision":
        ...
        # If the target element was explicitly identified in current perception by
        # element_id, bypass the servo entirely — perception already confirmed the
        # element exists at these coordinates, so a blank variance reading means the
        # element has a solid/uniform surface (button, dialog sidebar, icon), not that
        # the coordinate is empty space.  This covers Save-dialog buttons, navigation
        # links, toolbar icons, and other elements with low pixel variance.
    ...
    def _block_redundant_action(self, state, action, step_index: int):
        ...
        # Skip for perception-confirmed elements (is_input_zone=True): dialog sidebar
        # navigation and small UI interactions can produce < 1% screen change without
        # being stuck — the element was confirmed present by perception this step.
        if (
            not action.is_input_zone
            and progress_state.last_screen_change_ratio < self._STALL_SCREEN_CHANGE_THRESHOLD
