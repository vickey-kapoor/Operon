...
from operon.agent.actions.subgoal_utils import wrap_subgoal
...
class RuleBasedRecoveryManager(RecoveryManager):
    ...
    MAX_CONSECUTIVE_UNCERTAIN_TYPE_STEPS = 3
    ...
    async def recover(
        ...
        if self._uncertain_type_loop_detected(state, executed_action, verification):
            return RecoveryDecision(
                strategy=RecoveryStrategy.STOP,
                message=(
                    "Repeated uncertain verification after type actions; stopping instead "
                    "of continuing to re-type into changing targets."
                ),
    ...
    def _uncertain_type_loop_detected(
        self,
        state: AgentState,
        executed_action: ExecutedAction,
        verification: VerificationResult,
        ...
        if (
            executed_action.action.action_type is not ActionType.TYPE
            or not executed_action.success
            or verification.failure_category is not FailureCategory.UNCERTAIN_SCREEN_STATE
        ):
            return False

        pairs = list(zip(state.action_history, state.verification_history, strict=False))
        pairs.append((executed_action, verification))
        consecutive = 0
        for action_record, verification_record in reversed(pairs):
            if (
                action_record.success
                and action_record.action.action_type is ActionType.TYPE
                and verification_record.failure_category is FailureCategory.UNCERTAIN_SCREEN_STATE
            ):
                consecutive += 1
                if consecutive >= self.MAX_CONSECUTIVE_UNCERTAIN_TYPE_STEPS:
                    return True
            else:
                break
        return False
