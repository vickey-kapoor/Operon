...
Usage:
    python -m operon.store.cleanup            # dry-run: show what would be deleted
    python -m operon.store.cleanup --delete   # actually delete (keeps last 7 days)
    python -m operon.store.cleanup --days 3   # keep runs from the last 3 days
    python -m operon.store.cleanup --days 0   # keep ONLY today's runs
"""
