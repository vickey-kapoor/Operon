...
def test_submit_browser_task_safe_mode_skips_cdp_and_background_loop(client: TestClient) -> None:
    ...
    with (
        patch("operon.api.routes.browser.get_agent_loop") as mock_loop,
        patch("operon.api.routes.browser.ensure_cdp_ready", new_callable=AsyncMock) as mock_cdp,
        patch("operon.api.routes.browser.schedule_auto_run") as mock_schedule_auto_run,
    ):
        mock_loop.return_value.start_run = AsyncMock(return_value=_run_response())
        resp = client.post("/run-task", json={"intent": "Navigate to example.com"})
    ...
    mock_schedule_auto_run.assert_not_called()
...
async def test_schedule_auto_run_deduplicates_active_task(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    tasks._auto_run_tasks.clear()
    release = asyncio.Event()

    async def fake_auto_run(_loop, _run_id: str, _max_steps: int) -> None:
        await release.wait()

    monkeypatch.setattr(tasks, "auto_run_loop", fake_auto_run)

    first = tasks.schedule_auto_run(object(), "run-dedupe", 5)
    second = tasks.schedule_auto_run(object(), "run-dedupe", 200)

    assert first is second
    assert tasks._auto_run_tasks["run-dedupe"] is first

    await tasks.cancel_auto_run("run-dedupe")
    assert "run-dedupe" not in tasks._auto_run_tasks
...
async def test_schedule_auto_run_removes_completed_task(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    task = tasks.schedule_auto_run(object(), "run-complete", 5)
    await task
    await asyncio.sleep(0)

    assert "run-complete" not in tasks._auto_run_tasks
...
def test_stop_current_run_before_new_task(client: TestClient) -> None:
    ...
    with (
        patch("operon.api.routes.browser.get_agent_loop") as mock_loop,
        patch("operon.api.routes.browser.cancel_auto_run", new_callable=AsyncMock) as mock_cancel,
    ):
